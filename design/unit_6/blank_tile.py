...
def add_suffixes(hand, pre, start, row, results, anchored=True):
  ...
  if pre.upper() in PREFIXES:
    ...
    if is_letter(sq):
      ...
      for L in hand:
        # mushroom: L could be lower, cast to Upper case to check.
...
def horizontal_plays(hand, board):
  ...
  for (j, row) in enumerate(board[1:-1], 1):
    ...
    all_hands_list = list(itertools.product(*map(foo, list(hand))))
    all_hands = [''.join(h) for h in all_hands_list]
    for cur_hand in all_hands:
      for (i, word) in row_plays(cur_hand, row):
        score = calculate_score(board, (i, j), ACROSS, cur_hand, word)
...
def best_play(hand, board):
  ...
  print plays
  best = sorted(plays)[-1] if plays else NOPLAY
  print 'BEST is:', best
  return best
...
def test():
  ...
  assert ok('IKCEHKN', 80, (9, 1), (1, 0), 'KiCk')
