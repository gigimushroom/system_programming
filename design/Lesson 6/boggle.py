...
def find_words(board, word, index, result, visited_set):
  ...
  if word in PREFIXES:
    visited_set.add(index)  # Mark as visited if it is prefix.
    new_word = word + board[index]
    for n in neighbors(index, size(board)):
      cache = set(visited_set)
      find_words(board, new_word, n, result, visited_set)
      visited_set = set(cache)
  else:
    return
...
def boggle_words(board, minlength=3):
  ...
  for i,w in enumerate(board):
    ...
    results = set()
    visited_set = set()
    find_words(board, '', i, results, visited_set)
    finals |= results

  finals = set([m for m in list(finals) if len(m) >= minlength])
...
||||||""".strip()
  ...
  other_set = {'LID', 'SIR', 'OAR', 'LIS', 'RAG', 'SAL', 'RAM', 'RAW', 'SAY', 'RID', 'RIA', 'THO', 'HAY', 'MAR', 'HAS',
               'AYS', 'PHI', 'OIL', 'MAW', 'THIS', 'LAY', 'RHO', 'PHT', 'PLAYS', 'ASIDE', 'ROM', 'RIDE', 'ROT', 'ROW',
               'MAG', 'THIRD', 'WOT', 'MORE', 'WOG', 'WORE', 'SAID', 'MOR', 'SAIL', 'MOW', 'MOT', 'LAID', 'MOA', 'LAS',
               'MOG', 'AGO', 'IDS', 'HAIR', 'GAME', 'REM', 'HOME', 'RED', 'WORD', 'WHA', 'WHO', 'WHOM', 'YID', 'DRAW',
               'WAG', 'SRI', 'TOW', 'DRAG', 'YAH', 'WAR', 'MED', 'HIRE', 'TOWARDS', 'ORS', 'ALT', 'ORE', 'SIDE', 'ALP',
               'ORA', 'TWA', 'ERS', 'TOR', 'TWO', 'AIS', 'AIR', 'AIL', 'ERA', 'TOM', 'AID', 'TOG', 'DIS', 'HIS', 'GAR',
               'GAM', 'HID', 'HOG', 'PLAY', 'GOA', 'HOW', 'HOT', 'WARM', 'GOT', 'IRE', 'GOR', 'ARS', 'ARM', 'ARE',
               'TOWARD', 'THROW'}

  res = boggle_words(Board('PLAY THIS WORD GAME'))
  #print res ^ other_set

  assert boggle_words(Board('PLAY THIS WORD GAME')) == other_set
  return 'tests pass'
...
def display(board):
  ...
  N = size(board)
  #print N, [i for i in range(0, N ** 2, N)]
