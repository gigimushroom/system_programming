...
def anagrams(phrase, shortest=2):
  ...
  find_anagrams(phrase, results, all_matched)
  if all_matched:
    print all_matched
...
def find_anagrams(letters, results, all_matched):
  if letters == '':
    all_matched.add(' '.join(sorted(list(results))))
    return

  words = find_words(letters)
  for w in words:
    results.add(w)
    find_anagrams(removed(letters, w), results, all_matched)
    results.remove(w)
