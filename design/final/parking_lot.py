...
def is_goal(state):
  for (c, squares) in state:
    if c == '*' and 30 in squares:
      show(state)
      return True
    if c == '*' and 31 in squares:
      return True
  return False

def calc_new_state(car, locs, intr_range, allowed_cells, state, intr=1, debug=False):
  if not intr_range: return {}

  results = {}
  explored = set()
  explored.add(intr_range[0] - intr)
  for i in intr_range:
    moved_pos = set([l + i for l in locs])
    new_state = set(state)
    # We process if new locations are in empty cells and previous pos is explored.
    # Ex: if increment by 1 is a valid move, then we will consider incr by 2.
    if moved_pos <= allowed_cells and i - intr in explored:
      explored.add(i)
      # new locations are still in emtpy spaces, valid.
      action = (car, i)
      # Save this new state and action
      new_state.remove((car, tuple(locs)))
      new_state.add((car, tuple(moved_pos)))
      results[tuple(new_state)] = action
...
def psuccessors(state):
  """
  thoughts: for each car tuple
  move up, down, left, right
  if can move, try more one more step in same direction. Max is N - 3 (5)
  gather new state, action to a list
  """
  #show(state)
  # Remove all cars, walls, from all cells. I need empty spaces.
  empty_cells = set(range(0, N*N))
  cars_dict = {}
  for (c, squares) in state:
    empty_cells -= set(squares)
    if c != '@' or c != '|':
      # * is also a car, so all cars are stored.
      cars_dict[c] = squares
  ...
  results = {}
  # for each car
  for k, v in cars_dict.iteritems():
    # handle 4 cases
    allowed_cells = empty_cells | set(v)
    # right
    right_range = list(range(1, N - 2 - v[0] % N + 1))
    results.update(calc_new_state(k, v, right_range, allowed_cells, state))

    # left
    left_range = list(range(1, v[0] % N))
    left_range = [-n for n in left_range]
    results.update(calc_new_state(k, v, left_range, allowed_cells, state, intr=-1))

    # up
    top_range = list(range(v[0] % N + N - v[0], 0, N))
    top_range.reverse()
    results.update(calc_new_state(k, v, top_range, allowed_cells, state, intr=-8))

    # down
    down_range = list(range(v[0] + N, N * (N-1) + v[0] % N, 8))
    down_range = [x - v[0] for x in down_range]
    results.update(calc_new_state(k, v, down_range, allowed_cells, state, intr=8))

  #print results
  return results

def test_successor():
  assert path_actions(psuccessors(puzzle1_base)) == \
         [('A', -1), ('A', -2), ('A', -3), ('A', -8), ('A', 8),
          ('B', 1), ('B', -8), ('B', 8), ('B', 16), ('G', 1), ('G', 2), ('G', 3),
          ('*', -8), ('*', 8), ('*', 16), ('*', 24), ('O', 1), ('O', 2), ('O', 3),
          ('Y', -1), ('Y', 8)]

  print 'Successor Passed.'
...
def solve_parking_puzzle(start, N=N):
  ...
  path = shortest_path_search(start, psuccessors, is_goal)
  return path
...
def grid(cars, N=N):
  ...
  # given cars location, find wall locations
  cars = (('@', (31,)),) + cars

  wall_loc = [i for i in range(N)]
  wall_loc.extend([i for i in range(N*(N-1), N*N-1)])
  left_corner = [i for i in range(N, N*(N-1)+1, 8)]
  right_corner = [i for i in range(N-1, N * N + 1, 8) if i != 31]
  wall_loc.extend(left_corner)
  wall_loc.extend(right_corner)
  wall_loc = set(wall_loc)

  result = cars + (('|', tuple(wall_loc)),)
  return result
...
def test_parking():
  ...
  assert valid_solution(puzzle2, 3)
  assert valid_solution(puzzle3, 3)
  assert valid_solution(puzzle4, 6)
  assert locs(26, 2) == (26, 27)
...
def valid_solution(puzzle, length):
  ...
  print 'result', path_actions(path)
